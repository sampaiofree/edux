    ...
    <section class="space-y-6">
        <header class="rounded-card bg-white p-6 shadow-card flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <h1 class="font-display text-3xl text-edux-primary">Editar matricula #{{ $enrollment->id }}</h1>
                <p class="text-slate-600 text-sm">Atualize o curso, aluno e o progresso do estudante.</p>
            </div>
        ...
        <!--<form method="GET" action="{{ route('admin.enroll.edit', $enrollment) }}" class="rounded-card bg-white p-6 shadow-card space-y-4">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('admin.enroll.edit', $enrollment) }}" class="edux-btn bg-white text-edux-primary">Limpar filtros</a>
            </div>
        ...
        <form method="POST" action="{{ route('admin.enroll.update', $enrollment) }}" class="rounded-card bg-white p-6 shadow-card space-y-5">
            ...
            <div class="flex flex-wrap gap-3">
                <button type="submit" class="edux-btn">Salvar alteracoes</button>
