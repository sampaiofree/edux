                        ...
                        @foreach ($checkouts as $checkout)
                            <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 px-4 py-3">
                                <div>
                                    ...
                                    @if ($checkout->nome)
                                        <p class="mt-1 text-xs font-semibold text-gray-700">{{ $checkout->nome }}</p>
                                    @endif
                                    @if ($checkout->descricao)
                                        <p class="mt-1 text-xs leading-4 text-gray-500">{{ $checkout->descricao }}</p>
                                    @endif
                                </div>
                                ...
                                @if ($checkout->bonuses->isNotEmpty())
                                    <div class="w-full rounded-lg border border-gray-100 bg-gray-50 p-3">
                                        <p class="text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                                            Itens extras deste checkout
                                        </p>
                                        <div class="mt-2 space-y-2">
                                            @foreach ($checkout->bonuses as $bonus)
                                                <div class="rounded-lg bg-white px-3 py-2">
                                                    <div class="flex items-start justify-between gap-3">
                                                        <div>
                                                            <p class="text-xs font-semibold text-gray-800">{{ $bonus->nome }}</p>
                                                            @if ($bonus->descricao)
                                                                <p class="mt-1 text-[11px] leading-4 text-gray-500">{{ $bonus->descricao }}</p>
                                                            @endif
                                                        </div>
                                                        <span class="whitespace-nowrap text-xs font-bold text-yellow-700">
