    ...
    <article class="space-y-6 md:space-y-8">
                    ...
                    <div class="grid gap-0 md:grid-cols-[1.08fr_0.92fr]">
                        <div class="p-5 md:p-7">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="inline-flex items-center rounded-full bg-blue-50 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-edux-primary ring-1 ring-blue-100">
                                    Consulta local
                                </span>
                                <span class="inline-flex items-center rounded-full bg-amber-50 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-amber-800 ring-1 ring-amber-200">
                                    Valor social
                                </span>
                                <span class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-slate-700 ring-1 ring-slate-200">
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500">
                                <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1">
                                    <x-ui.color-icon name="map-pin" tone="blue" size="sm" class="h-5 w-5 rounded-md" />
                                    {{ $cityDisplayName }}
                                </span>
                                <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1">
                                    <x-ui.color-icon name="briefcase" tone="green" size="sm" class="h-5 w-5 rounded-md" />
                                    {{ $coursesCountLabel }}
                                </span>
                            </div>

                            <h1 class="mt-5 text-3xl font-black leading-tight tracking-tight text-slate-900 md:text-4xl">
                                Cursos com valor social liberados para {{ $cityDisplayName }}
                            </h1>

                            <p class="mt-3 text-sm leading-6 text-slate-600 md:text-base">
                                Veja os cursos disponíveis e escolha o seu caminho de capacitação profissional com acesso online, certificado e apoio para começar.
                            </p>
                            ...
                            <ul class="mt-5 space-y-3">
                                <li class="flex items-start gap-3 text-sm font-semibold text-slate-700">
                                    ...
                                    <span>Inscrição simplificada pelo CPF</span>
                                </li>
                                <li class="flex items-start gap-3 text-sm font-semibold text-slate-700">
                                    <x-ui.color-icon name="badge-check" tone="blue" size="sm" class="mt-0.5 h-6 w-6 rounded-md" />
                                    <span>Certificado incluso no valor do curso</span>
                                </li>
                                <li class="flex items-start gap-3 text-sm font-semibold text-slate-700">
                            ...
                            <div class="mt-6 flex flex-col gap-2 sm:flex-row">
                                <a
                                    href="#cursos-cidade"
                                    data-city-cta
                                    data-city-cta-source="hero_primary_v2"
                                    class="inline-flex min-h-[50px] items-center justify-center rounded-xl bg-edux-primary px-4 py-3 text-center text-sm font-black text-white transition hover:opacity-95"
                                >
                                ...
                                <a
                                    ...
                                    data-city-cta
                                    data-city-cta-source="hero_secondary_v2"
                                    class="inline-flex min-h-[50px] items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-center text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
                                >
                                    Como funciona
                                </a>
                            </div>
                            ...
                            @if ($showEmploymentDisclaimer)
                                <p class="mt-4 text-xs leading-5 text-slate-500">
                                    Iniciativa social de capacitação online. Não garante emprego, mas ajuda na sua preparação profissional.
                                </p>
                            @endif
                        </div>

                        <aside class="border-t border-slate-200 bg-slate-50 p-5 md:border-l md:border-t-0 md:p-7">
                            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Status da consulta</p>
                                        <h2 class="mt-1 text-lg font-black leading-tight text-slate-900">Liberação local ativa</h2>
                                    </div>
                                    <span class="inline-flex items-center rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold uppercase tracking-wide text-emerald-700 ring-1 ring-emerald-200">
                                        Ativa
                                    </span>
                                </div>
                                ...
                                <div class="mt-4 space-y-2">
                                    ...
                                    <div class="city-status-row">
                                        ...
                                        <strong>{{ $countdownEndsAtLabel }}</strong>
                                    </div>
                                </div>
                                ...
                                <p class="mt-3 text-center text-[11px] font-medium text-slate-500">
                                    Janela de {{ $countdownHours }}h para consulta local e acesso aos cursos.
                                </p>
                            </div>
                        </aside>
                    </div>
        ...
        @if (! $isClosed)
            <section id="como-funciona-cidade" class="city-campaign-section city-campaign-deferred rounded-3xl border border-edux-line/70 bg-white p-4 shadow-sm md:p-6">
                <div class="space-y-2">
                    <h2 class="text-2xl font-display text-edux-primary">Como funciona a consulta de cursos em {{ $cityDisplayName }}</h2>
                    <p class="text-sm text-slate-600 md:text-base">Veja os cursos liberados, abra a página oficial e escolha sua matrícula com segurança.</p>
                </div>
            ...
            <section id="cursos-cidade" class="city-campaign-section city-campaign-deferred py-4">
                <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Consulta local de cursos organizada pelo menor valor social.</p>
                    </div>
                ...
                @if ($courses->isNotEmpty())
                    <div class="grid gap-4 md:grid-cols-2">
                        @foreach ($courses as $courseItem)
                            <article class="flex flex-col overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg">
                                ...
                                <div class="p-5">
                                    ...
                                    <p class="mt-2 text-[11px] text-slate-400">
                                        Detalhes e matrícula na página oficial do curso.
                                    </p>

                                    <a
                                        href="{{ $courseItem['course_url'] }}"
                                        data-city-cta
                                        data-city-cta-source="course_row_v2"
                                        ...
                                        class="mt-4 flex w-full items-center justify-center rounded-2xl bg-slate-900 py-3.5 text-xs font-black uppercase tracking-widest text-white transition hover:bg-slate-800"
                                    >
                                        Ver página do curso
                                    </a>
                                </div>
            ...
            <section class="city-campaign-section city-campaign-deferred overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="grid gap-0 md:grid-cols-[1.02fr_0.98fr]">
                    <div class="order-2 p-5 md:order-1 md:p-6">
                        ...
                        <p class="mt-3 text-sm leading-6 text-slate-600 md:text-base">
                            Ao concluir o curso, você pode emitir seu certificado para comprovar sua capacitação e fortalecer seu currículo.
                        </p>
                        ...
                        <a
                            href="#cursos-cidade"
                            data-city-cta
                            data-city-cta-source="certificate_section_v2"
            ...
            <section class="city-campaign-section city-campaign-deferred rounded-3xl border border-amber-100 bg-gradient-to-br from-white via-white to-amber-50/50 p-4 shadow-sm md:p-6">
                <div class="grid gap-4 md:grid-cols-[1.1fr_0.9fr] md:items-start">
                    <div>
                        ...
                        <p class="mt-3 text-sm leading-6 text-slate-600 md:text-base">
                            A carta de estágio pode apoiar sua apresentação profissional junto ao currículo e candidaturas, mostrando seu compromisso com a capacitação.
                        </p>
                    ...
                    <div class="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm">
                        ...
                        <a
                            href="#cursos-cidade"
                            data-city-cta
                            data-city-cta-source="carta_estagio_section_v2"
                        ...
                        >
                            Ver página do curso e detalhes
                        </a>
                    </div>
                </div>
            </section>
            ...
            <section class="city-campaign-section city-campaign-deferred rounded-2xl border border-blue-100 bg-blue-50/50 p-4 md:p-5">
                <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div class="space-y-2">
                        <p class="text-sm font-black text-slate-900">Comunicado local para {{ $cityDisplayName }}</p>
                    ...
                    <a
                        href="#cursos-cidade"
                        data-city-cta
                        data-city-cta-source="footer_primary_v2"
    ...
    <script>
        (() => {
            const campaignMeta = @js([
                'page_type' => $cityTrackingPageType ?? 'city_campaign_catalog_v2',
                'city_slug' => $citySlug,
