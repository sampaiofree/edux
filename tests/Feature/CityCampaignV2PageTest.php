...
class CityCampaignV2PageTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_renders_open_city_catalog_v2_and_sets_shared_countdown_cookie(): void
    {
        ...
        $response = $this->get('/cidade-2/belo-horizonte?utm_source=meta');

        $response->assertOk();
        $response->assertSee('Cursos com valor social liberados para Belo Horizonte', false);
        $response->assertSee('Ver cursos liberados', false);
        $response->assertSee('data-city-countdown', false);
        $response->assertSee('data-city-cta-source="hero_primary_v2"', false);
        $response->assertSee('city_campaign_catalog_v2', false);
        $response->assertCookie($this->countdownCookieName('belo-horizonte'));
    }

    public function test_v2_uses_existing_countdown_cookie_from_variant_a(): void
    {
    ...
    public function test_v2_renders_closed_state_after_countdown_expires(): void
    {
    ...
    public function test_v2_lists_only_published_courses_and_preserves_variant_campaign_query_on_course_links(): void
    {
        ...
        $response = $this->get('/cidade-2/sao-paulo?utm_campaign=teste-v2');
        ...
        $response->assertSee('edux_city_variant=v2', false);
        $response->assertSee('utm_campaign=teste-v2', false);
        $response->assertSee(route('courses.public.show', $publishedA), false);
        $response->assertSee('data-city-cta-source="course_row_v2"', false);
    }

    public function test_v2_orders_courses_by_lowest_checkout_price_ascending(): void
    {
        ...
        $response = $this->get('/cidade-2/salvador');
